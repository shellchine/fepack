...
var execSync = exports.execSync = function(command, options){
    return proc.execSync(command, $$.extend({encoding: "utf-8", killSignal:"SIGPIPE"}, options));
}
var skipFiles = {};
...
//所有项目路径
var allPaths = {};
var tmp = read("/var/www/go", "path.conf");
tmp.split(/\n/).forEach(line => {
    if(/(\S+)\s+(\S+)/.test(line)){
...
function isValidFile(file){
    if (/^(gulpfile\.|webpack\.config)/.test(file) || skipFiles[file]) {
        ...
        if (/\.(s?html?|xml|vm|md|manifest)$/.test(file)) { //html, shtml
            return 9;
