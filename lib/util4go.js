...
var syncTo = exports.syncTo = conf.syncs[ENV.GO_SYNC_TO];
if (syncTo) {
...
function fulldir(file, dir){
    ...
    if(!dir){
        host = conf.devHost;
        dir = global.IMGURL_ROOT || '/' + vc.cdnfix + vc.path;
    }else{
...
function cdnImgPath(origPath, quote){
    ...
    if (global.IMGURL_ROOT) {    //外链css目录
        url = fulldir(url, global.IMGURL_ROOT);
    } else if (global.VCPATH) {
        url = fulldir(url);
    }
                ...
                if(new RegExp(`${conf.devHost}/${vc.cdnfix}${vc.path}/(.*)\\.(js|css)$`).test(url)){
                    ...
                    if(fs.existsSync(workfile)){
                        if(noSuffix(workfile)){
                            ...
                            url = vc.cdnpath+"/"+tmpfile+"."+vcver+"."+extname;
                        }
                    }
...
var procSingleCss = exports.procSingleCss = async function(f, compress, css){
...
var sync2backend = exports.sync2backend = function(distHtmlDir, rDir){ //通过rsync、scp等同步
    if (rDir) {
        log(`[syncTo] ${syncTo.type} to ${rDir}\n`);
        getFolder(rDir);
        if (!/devQA/i.test(stage)) {
            try{
                execSync(`${syncTo.type} -r ${distHtmlDir}/* ${rDir}/`);
                if(!global.distUtils.cdn){
                    execSync(`${syncTo.type} -r ${distStaticDirs[0]}/* ${rDir}/ 2>/dev/null`);
...
function fixIncPath(dir){
    ...
    lsr(dir).forEach(filename => {
        ...
        if (fs.existsSync(file) && !binaryReg.test(file)) {
            ...
            var envpath = global.VCPATH;
            if (subdir) {
                global.VCPATH = envpath+"/"+subdir;
            }
            if(/dev/i.test(stage)){
                //扩充相对路径，分别基于每个html所在目录
                html = expandFullPath(html);
            }
            global.VCPATH = envpath;
...
exports.cpRes = async function(workfile, base, flag){
    ...
    if(copyOnly){
        ...
        if (/\.css$/.test(base)) {
            //内联 @import
            global.IMGURL_ROOT = conf.devHost + path.resolve(`/${vc.path}`, dir);
            log(`cpRes ${workfile} (${global.IMGURL_ROOT}) .. `);
            [nsize, osize, count] = await procSingleCss(outputFile, conf.compress.css);
            global.IMGURL_ROOT = "";
            status = "done";
