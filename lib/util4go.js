                ...
                if(new RegExp(`${conf.devHost}/${vc.cdnfix}${vc.path}/(.*)\\.(js|css)$`).test(url)){
                    ...
                    if(fs.existsSync(workfile)){
                        if(noSuffix(workfile)){
                            ...
                            url = vc.cdnpath+"/"+tmpfile+"."+vcver + (postfix == 'js' && ENV.JS_COMPRESS ? ".min" : "") + "."+postfix;
                        }
                    }
...
var rsync2backend = exports.rsync2backend = function(distHtmlDir, rDir){
    if (rDir) {
        log(`Copy to ${rDir}\n`);
        getFolder(rDir);
...
exports.proc4backend = function(distHtmlDir, rDir){
    ...
    rsync2backend(distHtmlDir, rDir);
}
...
exports.cpRes = async function(workfile, base, flag){
    ...
    if(copyOnly){
        ...
        if (/\.css$/.test(base)) {
            ...
            [nsize, osize, count] = await procSingleCss(outputFile, ENV.CSS_COMPRESS);
            global.IMGURL_ROOT = "";
            status = "done";
